# Happy Homes Dealer Hub — Dealer Home (v4)
...
v4 replaces the v3 collapsible category sections with **Meridian-style
...
Unchanged on purpose: **the order review is still preview-only.** Nothing is
transmitted to Happy Homes from this screen, and both the drawer and the
review screen say so.
...
```
Google Sheet "happy homes inventory"  (3 tabs, 13-column schema)
        |
        |  Apps Script web app  (Code.gs, deployed /exec, access: Anyone)
        v
   JSON feed  { categories: [ { name, count, items:[ row objects ] } ] }
        |
        |  fetch() on page load
        v
   index.html  (single file, no dependencies, hosted on Netlify)
        |
        |  <iframe>
        v
...
Two files, no build step, no framework, no CDN.
...
project with no connected repo, so deploying means uploading the file:

1. Open <https://app.netlify.com/projects/dealerhappyhomes>.
2. **Deploys** tab → drag `index.html` onto the drop zone
   (drop the file itself, or a folder containing only it).
...
```js
var CONFIG = {
  feedUrl:        "https://script.google.com/macros/s/AKfycbx…/exec",
...
**Browser — 36 checks**: exactly 4 cards across at 1440px and 2 at 390px;
...
- **Every dealer currently sees the same `dealerName`.** It is a constant in the
  ...
  demo (see below).
- **The order review does not submit anywhere.** That is deliberate for now.
  Wiring it up is the single highest-value next step: an Assembly **Form**, or a
  Make webhook writing to an `orders` tab, would turn this from a catalogue into
  the paperwork-elimination story.
