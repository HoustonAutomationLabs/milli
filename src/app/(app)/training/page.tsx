...
export default async function TrainingPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {playable.length < TRAINING_LIBRARY.length ? (
        <Card className="mb-6 border-l-4" style={{ borderLeftColor: "var(--warn)" }}>
          <SectionTitle>
            {playable.length} of {TRAINING_LIBRARY.length} slots filled
          </SectionTitle>
          <p className="mt-3 max-w-3xl text-[15px] leading-relaxed text-ink-soft">
            Add a <code className="text-[13px]">source</code> to the remaining entry
            in <code className="text-[13px]">src/lib/training.ts</code> &mdash; either
            a file under <code className="text-[13px]">public/training</code> or an
            instagram.com permalink. Anything that is not a recognised post link is
      ...
      <Card className="mt-6">
        ...
        <ul className="mt-3 flex flex-col gap-2 text-[14px] leading-relaxed text-ink-soft">
          <li>
            These clips are served{" "}
            <strong className="font-semibold text-ink">from this app&rsquo;s own origin</strong>,
            not from a social platform. Nothing third-party is contacted when you
            open this page or play a video.
          </li>
          <li>
            Video data only downloads when you press play &mdash; until then a card
            has loaded a poster image and nothing else.
          </li>
          <li>
            An entry may instead link a post on the agency&rsquo;s Instagram, in
            which case it stays click-to-load: this app renders children&rsquo;s
            records, and an embed firing on page load would send a referrer from a
            clinical application to a third party on every visit.
          </li>
          <li>
